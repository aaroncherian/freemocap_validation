...
def add_histogram_overlay_panel(
    ...
    *,
    ...
    # ticks (match your wide-scale screenshot style)
    ...
    for label, dfi in dfs.items():
        ...
        # print μ ± σ (ms) for sanity checking
...
# ------------------------
# KEY CHANGE:
# Left = RTMPose, Right = MediaPipe-DLC
# and we overlay those in the SAME panels
# ------------------------

# stance
paired_stance_left_rt = get_paired_stride_df(
    gait_param_df, metric="stance_duration", side="left", fmc_system="rtmpose"
)
paired_stance_right_mp = get_paired_stride_df(
    gait_param_df, metric="stance_duration", side="right", fmc_system="mediapipe_dlc"
)

# swing
paired_swing_left_rt = get_paired_stride_df(
    gait_param_df, metric="swing_duration", side="left", fmc_system="rtmpose"
)
paired_swing_right_mp = get_paired_stride_df(
    gait_param_df, metric="swing_duration", side="right", fmc_system="mediapipe_dlc"
)
...
# legend entries (dummy traces) so we get a clean global legend
fig.add_trace(
    go.Scatter(
        x=[None], y=[None],
        mode="lines",
