...
# --- TOP-DOWN (X–Y) plotting helpers ---
def seg_line_xy(p1, p2, color, name=None, showlegend=False, width=5):
    return go.Scatter(
...
def trail_points_xy(P, color, name=None, showlegend=False):
    return go.Scatter(
...
# Collect global axis limits across all conditions + systems (X–Y only)
all_xy = []
...
for cond, root in conditions.items():
    ...
    # For axis ranges (stack X and Y columns 0:2)
    for sys_means in (FMC_means[cond], QTM_means[cond]):
        all_xy.append(sys_means['knee'][:,0:2])
        all_xy.append(sys_means['ankle'][:,0:2])
        all_xy.append(sys_means['toe'][:,0:2])
        all_xy.append(sys_means['heel'][:,0:2])
...
for cond, color in COND_COLORS.items():
    ...
    if SHOW_TRAIL:
        fig.add_trace(trail_points_xy(means['toe'][:t0+1], color, name=name, showlegend=False), row=1, col=1)
    else:
        fig.add_trace(trail_points_xy(np.zeros((0,3)), color, name=name, showlegend=False), row=1, col=1)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['knee'][t0], color), row=1, col=1)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['toe'][t0],  color), row=1, col=1)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['heel'][t0], color), row=1, col=1)
    fig.add_trace(go.Scatter(x=[means['ankle'][t0,0]], y=[means['ankle'][t0,1]], mode="markers",
...
for cond, color in COND_COLORS.items():
    ...
    if SHOW_TRAIL:
        fig.add_trace(trail_points_xy(means['toe'][:t0+1], color, name=name, showlegend=False), row=1, col=2)
    else:
        fig.add_trace(trail_points_xy(np.zeros((0,3)), color, name=name, showlegend=False), row=1, col=2)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['knee'][t0], color), row=1, col=2)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['toe'][t0],  color), row=1, col=2)
    fig.add_trace(seg_line_xy(means['ankle'][t0], means['heel'][t0], color), row=1, col=2)
    fig.add_trace(go.Scatter(x=[means['ankle'][t0,0]], y=[means['ankle'][t0,1]], mode="markers",
                             marker=dict(size=7, color=color), showlegend=False,
...
# Layout / axes
fig.update_layout(
    title="Gait-AVERAGED right leg top-down (X–Y) per condition (Left: FMC, Right: QTM)",
)
...
fig.update_xaxes(title_text="X (Mediolateral)", range=[xmin, xmax], row=1, col=2)
fig.update_yaxes(title_text="Y (Anterior–Posterior)", range=[ymin, ymax], row=1, col=2, scaleanchor="x2", scaleratio=1)
...
for i in range(NPTS):
    ...
    for cond, color in COND_COLORS.items():
        m = FMC_means[cond]
        data_list.append(trail_points_xy(m['toe'][:i+1] if SHOW_TRAIL else np.zeros((0,3)), color))
        data_list.append(seg_line_xy(m['ankle'][i], m['knee'][i], color))
        data_list.append(seg_line_xy(m['ankle'][i], m['toe'][i],  color))
        data_list.append(seg_line_xy(m['ankle'][i], m['heel'][i], color))
        data_list.append(go.Scatter(x=[m['ankle'][i,0]], y=[m['ankle'][i,1]], mode="markers",
                                    marker=dict(size=7, color=color), showlegend=False))
...
def add_condition_legend(fig, cond_colors):
    """Add one invisible trace per condition so the legend shows each condition once."""
    for cond, color in cond_colors.items():
        fig.add_trace(
            go.Scatter(
                x=[None], y=[None],
...
# If you want anterior to appear "up" on the page but your Y increases anteriorly already, keep as is.
# If your Y increases posteriorly, flip with:
