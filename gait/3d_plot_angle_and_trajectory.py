...
# Axis limits (Y–Z only, top plot)
stack = np.vstack([
    FMC['knee'][:,1:], FMC['ankle'][:,1:], FMC['toe'][:,1:], FMC['heel'][:,1:],
    QTM['knee'][:,1:], QTM['ankle'][:,1:], QTM['toe'][:,1:], QTM['heel'][:,1:]
])
...
def yz_line_segment(p1, p2, color, name, width=5, showlegend=False):
    return go.Scatter(
...
def yz_points(P, color, name, size=6, showlegend=False, opacity=0.6):
    return go.Scatter(
...
fig.add_trace(
    yz_points(FMC['toe'][:t0+1] if SHOW_TRAIL else np.zeros((0,3)),
...
# 5) QTM toe trail
fig.add_trace(
    yz_points(QTM['toe'][:t0+1] if SHOW_TRAIL else np.zeros((0,3)),
              COL_QTM, "QTM toe trail", size=3, showlegend=True),
    row=1, col=1
)

# 6) FMC shank
fig.add_trace(yz_line_segment(FMC['ankle'][t0], FMC['knee'][t0], COL_FMC, "FMC shank", showlegend=True), row=1, col=1)
# 7) FMC foot
fig.add_trace(yz_line_segment(FMC['ankle'][t0], FMC['toe'][t0],  COL_FMC, "FMC foot"), row=1, col=1)
# 8) FMC heel (same color)
fig.add_trace(yz_line_segment(FMC['ankle'][t0], FMC['heel'][t0], COL_FMC, "FMC heel"), row=1, col=1)
# 9) FMC ankle marker
fig.add_trace(go.Scatter(x=[FMC['ankle'][t0,1]], y=[FMC['ankle'][t0,2]], mode="markers",
                         marker=dict(size=7, color=COL_FMC), name="FMC ankle", showlegend=False), row=1, col=1)

# 10) QTM shank
fig.add_trace(yz_line_segment(QTM['ankle'][t0], QTM['knee'][t0], COL_QTM, "QTM shank", showlegend=True), row=1, col=1)
# 11) QTM foot
fig.add_trace(yz_line_segment(QTM['ankle'][t0], QTM['toe'][t0],  COL_QTM, "QTM foot"), row=1, col=1)
# 12) QTM heel
fig.add_trace(yz_line_segment(QTM['ankle'][t0], QTM['heel'][t0], COL_QTM, "QTM heel"), row=1, col=1)
# 13) QTM ankle marker
fig.add_trace(go.Scatter(x=[QTM['ankle'][t0,1]], y=[QTM['ankle'][t0,2]], mode="markers",
                         marker=dict(size=7, color=COL_QTM), name="QTM ankle", showlegend=False), row=1, col=1)

# Layout / axes
fig.update_layout(
    title="Gait-AVERAGED right leg in Y–Z + ankle flex/ext over %GC (blue=FMC, red=QTM)",
    annotations=angle_annotation(0),
    legend=dict(orientation="h", yanchor="bottom", y=1.04, xanchor="right", x=1.0),
)
fig.update_xaxes(title_text="Y (Anterior–Posterior)", range=[ymin, ymax], row=1, col=1)
...
# ---------- BOTTOM PLOT (row=2): static lines + moving cursor ----------
gc = np.linspace(0, 100, NPTS)
t0 = 0
...
# ---------- Frames (update top view + bottom cursor + annotation) ----------
from plotly.subplots import make_subplots

# ---------- Toe-Z subplot limits (row=2) ----------
FMC_toe_z = FMC['heel'][:, 2]
QTM_toe_z = QTM['heel'][:, 2]
tz_all = np.concatenate([FMC_toe_z, QTM_toe_z])
tzmin = float(np.nanmin(tz_all))
tzmax = float(np.nanmax(tz_all))
tzpad = 0.05 * (tzmax - tzmin + 1e-6)
tzmin -= tzpad
tzmax += tzpad
...
fig = make_subplots(
    rows=3, cols=1,
    row_heights=[0.54, 0.23, 0.23],  # top: YZ schematic, mid: Toe-Z, bottom: angle
...
# ---------- Add static curves and initial cursor (consistent order) ----------
gc = np.linspace(0, 100, NPTS)
t0 = 0

# (Row 3) Ankle angle FMC/QTM + cursor
fig.add_trace(
    go.Scatter(x=gc, y=FMC_angle, mode="lines",
               line=dict(width=3, color=COL_FMC),
               name="FMC ankle angle"),
...
fig.add_trace(
    go.Scatter(x=[cursor_x, cursor_x], y=[aymin, aymax], mode="lines",
               line=dict(width=2, dash="dash", color="black"),
               name="%GC cursor (angle)", showlegend=False),
    row=3, col=1
)

# (Row 2) Toe-Z FMC/QTM + cursor
fig.add_trace(
    go.Scatter(x=gc, y=FMC_toe_z, mode="lines",
               line=dict(width=3, color=COL_FMC),
               name="FMC toe Z"),
    row=2, col=1
)
fig.add_trace(
    go.Scatter(x=gc, y=QTM_toe_z, mode="lines",
               line=dict(width=3, color=COL_QTM),
               name="QTM toe Z"),
    row=2, col=1
)
fig.add_trace(
    go.Scatter(x=[cursor_x, cursor_x], y=[tzmin, tzmax], mode="lines",
               line=dict(width=2, dash="dash", color="black"),
               name="%GC cursor (toe Z)", showlegend=False),
    row=2, col=1
)

# (Row 1) Top Y–Z scene: toe trails + segments/markers at i=t0
fig.add_trace(
    yz_points(FMC['toe'][:t0+1] if SHOW_TRAIL else np.zeros((0,3)),
              COL_FMC, "FMC toe trail", size=3, showlegend=True),
    row=1, col=1
...
# Layout / axes
fig.update_layout(
    title="Gait-AVERAGED right leg in Y–Z + Toe Z and Ankle angle over %GC (blue=FMC, red=QTM)",
    annotations=angle_annotation(0),
...
fig.update_xaxes(title_text="% Gait Cycle", range=[0, 100], row=2, col=1)
fig.update_yaxes(title_text="Toe height Z [same units as data]", range=[tzmin, tzmax], row=2, col=1)

fig.update_xaxes(title_text="% Gait Cycle", range=[0, 100], row=3, col=1)
fig.update_yaxes(title_text="Ankle dorsiflexion (+) [deg]", range=[aymin, aymax], row=3, col=1)

# ---------- Frames (now exactly 16 traces in this order per frame) ----------
def trail_points(P, color):
    return go.Scatter(x=P[:,1], y=P[:,2], mode="markers",
                      marker=dict(size=3, color=color, opacity=0.6), showlegend=False)

def seg_line(p1, p2, color):
    return go.Scatter(x=[p1[1], p2[1]], y=[p1[2], p2[2]],
...
for i in range(NPTS):
    cursor_x = gc[i]
    data_list = [
        # (Row 3) Ankle angle static FMC/QTM + moving cursor
        go.Scatter(x=gc, y=FMC_angle, mode="lines",
        ...
        # (Row 2) Toe-Z static FMC/QTM + moving cursor
        go.Scatter(x=gc, y=FMC_toe_z, mode="lines",
                   line=dict(width=3, color=COL_FMC), showlegend=False),
        go.Scatter(x=gc, y=QTM_toe_z, mode="lines",
                   line=dict(width=3, color=COL_QTM), showlegend=False),
        ...
        # (Row 1) Y–Z scene: FMC/QTM toe trails + segments/markers at frame i
        trail_points(FMC['toe'][:i+1] if SHOW_TRAIL else np.zeros((0,3)), COL_FMC),
        trail_points(QTM['toe'][:i+1] if SHOW_TRAIL else np.zeros((0,3)), COL_QTM),

        seg_line(FMC['ankle'][i], FMC['knee'][i], COL_FMC),
        seg_line(FMC['ankle'][i], FMC['toe'][i],  COL_FMC),
        seg_line(FMC['ankle'][i], FMC['heel'][i], COL_FMC),
        go.Scatter(x=[FMC['ankle'][i,1]], y=[FMC['ankle'][i,2]], mode="markers",
                   marker=dict(size=7, color=COL_FMC), showlegend=False),

        seg_line(QTM['ankle'][i], QTM['knee'][i], COL_QTM),
        seg_line(QTM['ankle'][i], QTM['toe'][i],  COL_QTM),
        seg_line(QTM['ankle'][i], QTM['heel'][i], COL_QTM),
        go.Scatter(x=[QTM['ankle'][i,1]], y=[QTM['ankle'][i,2]], mode="markers",
...
# ---------- Controls (unchanged, just nudged positions a bit) ----------
...
fig.update_layout(
    updatemenus=[{
        ...
        "showactive":False, "x":0.02, "y":1.08, "xanchor":"left", "yanchor":"top"
    }],
    sliders=[{
        "active":0, "steps":steps, "x":0.20, "y":1.05, "xanchor":"left", "yanchor":"top",
