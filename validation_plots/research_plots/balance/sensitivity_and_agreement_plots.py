"""
Combined MediaPipe COM path-length figure for the eLife paper.

Merges the two former primary figures into one composite:
...
Top row tells the absolute-agreement story; bottom row tells the
"does MediaPipe track experimentally-induced change" story.

Both halves are computed from the same `path_length_com` artifacts, so the DB
is read once. Stat functions are lifted verbatim from the original agreement /
manipulation-effects scripts (renamed where they clashed) so the numbers match
...
if __name__ == "__main__":
    ...
    # fig.write_image(root_path / "com_path_length_mediapipe_combined.pdf")